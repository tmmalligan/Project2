...
void loop() {  
  ...
  Serial.print(" Temperature Colour Change ");
  temperature(); 
  delay(100);
  break;
...
void changeColour(){
 ...
 void temperature(){//Adjusts lights according to input from LDR sensor
  ...
  else if(tempF >= hot) // if temperature is hot
  {
    ...
    digitalWrite(blueLED, HIGH);        
    digitalWrite(greenLED, LOW);
    Serial.println("\t I am sweating! :O");      
  }
    ...
    digitalWrite(blueLED, LOW);   
    digitalWrite(greenLED, HIGH);
    Serial.println("\t sweater weather :')");  
    }
    delay(100);
 }
